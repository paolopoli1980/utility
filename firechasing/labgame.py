...
class Diamond:
    def __init__(self):
        ...
        self.Model = cylinder(pos= self.Pos, axis=(0,0,1), radius=0.5,color=color.magenta, opacity =0.5)
...
class Bullet:
    ...
    def __init__(self,pos,direction,velocity):
        self.Pos=pos
        self.Fire=sphere(pos=self.Pos,radius=0.1,color=color.yellow)
...
class Player:
    ...
    def changeposfromcell(self,XY):
        ...
        self.modelPlayer = pyramid(pos=(self.PosX,self.PosY,self.PosZ), size=(1,1,1),axis=(0.1,0,0),color=self.Colore)
    ...
    def __init__(self,tastileft,tastiright,tastiup,tastidown,fire,bonusVel,invis,colore):
        ...
        self.getCell()        
        self.modelPlayer = pyramid(pos=(self.PosX,self.PosY,self.PosZ), size=(1,1,1),axis=(0.1,0,0),color=self.Colore)
        self.ShipLabel = label(pos= self.modelPlayer.pos, text= str(self.Lives)) 
        ...
        for elem in key:
            if elem==self.Fire:               
                ...
                if ((diffdate > self.DeltaFire) and (self.CanFire==true) ):                
                    self.Bullets.append(Bullet(pos=vector(self.PosX,self.PosY,self.PosZ),direction=vector(self.direction),velocity=0.2))
    ...
    def moving(self):
        ...
        self.modelPlayer.pos=(self.PosX,self.PosY,self.PosZ)
...
plane=box (pos = vector(cube.lato*lenght/2-cube.lato/2,cube.lato*large/2-cube.lato/2,-cube.lato/2), length=cube.lato*lenght, height=cube.lato*large, width=0.1,color=(0,0,255))    
for cubo in wall:
    Cubo=box (pos = vector(cubo.X,cubo.Y,0), length=cube.lato, height=cube.lato, width=cube.lato,color=(100,100,0))
